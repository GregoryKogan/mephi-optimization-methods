...
Все требуемые библиотеки для _Python_ записаны в [requirements.txt](scripts/requirements.txt). Для установки нужно выполнить в терминале(находясь в папке с проектом) следующую команду:
...
Установите минимальную версию _TeXLive_, занимающую минимальное количество места на жестком диске:

#### Unix

Пример установки через пакетный менеджер для _Debian_/_Ubuntu_:
...
Убедитесь, что _tlmgr_ доступен в переменных окружения _PATH_, чтобы его можно было запустить из командной строки или _PowerShell_.

Все требуемые библиотеки для _TeXLive_ записаны в [tex-requirements.txt](/tex_requirements.txt).

Для установки на _Unix_/_MacOS_ необходимо выполнить следующую команду:

```bash
sudo xargs tlmgr install < path_to_project/tex_requirements.txt
```

Для _Windows_ необходимо выполнить в _PowerShell_:
...
Для начала будет достаточно поменять название документа и автора в полях _title_ и _author_.
...
1. В папке **/src** располагаются файлы в формате _markdown_.
2. _Python_ скрипт [build.py](/scripts/build.py) формирует временный файл **metadata.yaml** из настроек в конфигурационном файле [build-conf.toml](/build-conf.toml).
3. Выполняется **bash** команда, создающая выходной файл.

Для соединения в единый PDF файл используется утилита [pandoc](https://pandoc.org/index.html),которая переводит _markdown_ файлы в единый _.tex_ файл, добавляет к нему шаблон и настройки и формирует единый PDF.
...
Запуск _Python_ скрипта создает выходной PDF:
...
[_eisvogel-custom.tex:_](/templates/eisvogel-custom.tex)
...
[_eisvogel-custom_mephi_titlepage.tex_](/templates/eisvogel-custom_mephi_titlepage.tex)
отличается от [_eisvogel-custom.tex:_](/templates/eisvogel-custom.tex)
